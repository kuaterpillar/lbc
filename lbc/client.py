...
class Client(
    ...
    def _fetch(self, method: str, url: str, payload: Optional[dict] = None, max_retries: int = -1) -> dict:
        ...
        if response.ok:
            ...
            if self.proxy:
                raise DatadomeError("Access blocked by Datadome: your proxy appears to have a poor reputation, try to change it.")
            else:
                raise DatadomeError("Access blocked by Datadome: your activity was flagged as suspicious. Please avoid sending excessive requests.")
